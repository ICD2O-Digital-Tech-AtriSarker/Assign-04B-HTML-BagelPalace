...
function addTopping(toppingName, pos = "default") {
  ...
  // Connect dropdown value change to the drawSandwich function
...
amountOfToppingsInput.oninput = () => {
  ...
  // This is done to display the sandwich with the newly added toppings from the dropdowns.
  drawSandwich();
}
...
function drawSandwich() {
  ...
  // Images are drawn from their top-left position
  ...
  if (zIndex >= toppingList.length ) {
    // Escape condition, returns once all toppings are drawn
    return
  }
